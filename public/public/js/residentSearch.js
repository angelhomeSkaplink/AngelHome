	  ...
	  inp.addEventListener("input", function(e) {
		  ...
		  for (i = 0; i < arr.length; i++) {
			...
			if (arr[i].name.substr(0, val.length).toUpperCase() == val.toUpperCase()) {
			  ...
			  b.innerHTML = "<span><img src=\"http://127.0.0.1:8000/img/"+arr[i].image+"\" class='img-circle' width='30' height='30'></span><strong>" + arr[i].name.substr(0, val.length) + "</strong>";
	...
	$.ajax({
		...
		success:function(data) {
			res_array = data;
			autocomplete(document.getElementById("myInput"), res_array);
