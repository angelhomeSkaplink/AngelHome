...
<div class="row">
	<form action="insert_sallary_temporary"  method="post">	
		{!! csrf_field() !!}	
        <div class="col-md-12">			
                    ...
                    <table class="table table-bordered">
                        <tbody>
                            ...
                            <tr>
								...
								<?php $loans = DB::table('attendances')->select(DB::raw('count(emp_id) as emp_count'))->where([['emp_id', $employee->emp_id], ['status', '1']])->get();	?>
								@forelse($loans as $l)
								<td><input type="hidden" class="form-control" placeholder="" name="emp_count[]" value="{{ $l->emp_count }}"  /></td>										
								@empty
								<td><input type="hidden" class="form-control" placeholder="" value="0" name="emi[]" /></td>
								@endforelse
							</tr>		
							@endforeach							
							<div class="row">                    
